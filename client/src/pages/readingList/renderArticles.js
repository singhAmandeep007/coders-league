      ...
      case "SEARCH":
        ...
        const matchedArticles = articleData.filter((article) => {
          if (searchTerm && searchTags.length === 0 && !searchExpertiseLevel) {
            ...
            return (
              article.title.indexOf(searchTerm) !== -1 && article.expertiseLevel.indexOf(searchExpertiseLevel) !== -1
            );
          } else if (!searchTerm && searchTags.length > 0 && searchExpertiseLevel) {
            return (
              article.expertiseLevel.indexOf(searchExpertiseLevel) !== -1 &&
              searchTags.every((v) => article.tags.includes(v))
            );
          }
          return (
            article.title.indexOf(searchTerm) !== -1 &&
            article.expertiseLevel.indexOf(searchExpertiseLevel) !== -1 &&
            searchTags.every((v) => article.tags.includes(v))
          );
        });
  ...
  return (
    <>
      <Grid.Column width={5}>
        <ArticleSearchWidget
          search={(searchTerm, searchTags, searchExpertiseLevel, sortBy) =>
            dispatch({ type: "SEARCH", payload: { searchTerm, searchTags, searchExpertiseLevel, sortBy } })
          }
      ...
      <Grid.Column width={11}>
        <div
          className="ui secondary black segment"
          style={{ padding: "1.4em" }}
        >
