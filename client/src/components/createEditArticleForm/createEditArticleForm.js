...
const CreateEditArticleForm = forwardRef(
  ({ currentUser, articleData = null }, ref) => {
    ...
    useEffect(() => {
      register(
        { name: 'expertiseLevel' },
        { required: 'Article must have an expertise level.' }
      );
      register(
        { name: 'tags' },
        {
          validate: (value) => {
            return (
              (1 <= value.length && value.length < 5) ||
              'You must add at least 1 tag and at most 4 tags in a article.'
            );
          },
        }
      );
              ...
              fetch('https://api.cloudinary.com/v1_1/dryiuvv1l/image/upload', {
                ...
                .then((response) => {
                  if (response.ok) {
                    ...
                    throw new Error(
                      'Something went wrong with uploading image!'
                    );
                  }
                })
                .then((result) => {
    ...
    return (
      ...
      >
        <Form.Field error={!!errors.file}>
              ...
              ref={register({
                validate: {
                  checkSize: (value) => {
                    if (value[0]) {
                      return (
                        value[0].size < 5242880 ||
                        'Only Image upto the size of 5MB is allowed!'
                      );
                    }
                    return;
                  },
          ...
          {errors.file && (
            <span style={formStyle.errorMessage}>{errors.file.message}</span>
          )}
        ...
        <Form.Field error={!!errors.title}>
          ...
          {errors.title && (
            <span style={formStyle.errorMessage}>{errors.title.message}</span>
          )}
        </Form.Field>
        <Form.Field error={!!errors.expertiseLevel}>
          ...
          {errors.expertiseLevel && (
            <span style={formStyle.errorMessage}>
        ...
        <Form.Field error={!!errors.tags}>
          ...
          />
          {errors.tags && (
            <span style={formStyle.errorMessage}>{errors.tags.message}</span>
          )}
        </Form.Field>
        ...
        <Form.Field error={!!errors.body}>
          ...
          />
          {errors.body && (
            <span style={formStyle.errorMessage}>{errors.body.message}</span>
          )}
        </Form.Field>
          ...
          >
            ...
            <Message.Content>
              ...
              {status.errors ? (
                ...
                <Link to={status.successMessage}>
                  Published 🚀. Click to visit it.
                </Link>
              )}
            </Message.Content>
          </Message>
