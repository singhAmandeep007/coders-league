  ...
  function reducer(state, action) {
    switch (action.type) {
      ...
      case "updateComment":
        return {
          ...state,
          commentsData: state.commentsData.map((comment) => {
    ...
    <>
      ...
      <Segment basic>
        <Comment.Group collapsed={state.collapsed}>
          ...
          {state.commentsData && state.commentsData.length > 0 ? (
            state.commentsData.map((comment) => {
              return (
                <CommentBox
                  ...
                  isLiked={
                    currentUser && comment.commentLikes && comment.commentLikes.users.indexOf(currentUser.id) !== -1
                      ? true
                      : false
                  }
