  ...
  return (
    <Segment raised>
      ...
      <Segment
        ...
        style={{ margin: "0em" }}
      >
        {articleData && articleData.title && <h1 className="ui dividing header">{articleData.title}</h1>}

        {articleData && articleData.expertiseLevel && (
          <Label
            key="expertiseLevel"
            color={
              articleData.expertiseLevel === "advanced"
                ? "red"
                : articleData.expertiseLevel === "intermediate"
                ? "orange"
                : "blue"
            }
          >
            <i className="graduation cap icon"></i>
            {articleData.expertiseLevel}
          </Label>
        )}
      ...
      {articleData && articleData.body && (
        <div className="ql-snow">
          <div className="ql-editor">{parse(articleData.body, options)}</div>
        </div>
      )}
    </Segment>
  );
