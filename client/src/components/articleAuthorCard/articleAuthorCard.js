...
const ArticleAuthorCard = ({ userData, isAuthenticated, currentUserId, setUserFollowing, userFollowing }) => {
   ...
   return (
      <Card fluid raised>
         ...
         <Card.Content>
            <img className="ui mini left floated image" src={userData.photo} alt={userData.username} />
            <Card.Header as={Link} to={`/u/${userData.username}`}>
               {userData.fullname}
            </Card.Header>
            ...
            {userData.bio && <Card.Description style={{ fontSize: '0.83rem', lineHeight: '1.5em', paddingBottom: '1em' }}>
               {userData.bio}
            </Card.Description>}
            {/* user cant follow himself */}
            {isAuthenticated && currentUserId !== userData.id && <button
